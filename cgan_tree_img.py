...
def parse_args():
    ...
    parser.add_argument('--context_dim', type=int, default=8, help='dimensionality of conditional information') 
...
#  use tree images of eight classes
import torchvision.transforms as transforms
from torchvision.datasets import ImageFolder

data_path= "/content/drive/MyDrive/001Coursework/ECE machine learning/cGAN"

def load_local_images(batch_size, train=True, shuffle=True, num_workers=4, pin_memory=True):
    ...
    data_folder = 'tree_gan_output/train' if train else 'tree_gan_output/test'
...
real_image_train = load_local_images(batch_size=100, train=True)
real_image_test = load_local_images(batch_size=100, train=False)


 
def get_batch(batch_size, device):
    real_image, label = next(iter(real_image_train))
    real_image = real_image.to(device)
    label = torch.nn.functional.one_hot(label, num_classes=8).type(torch.float).to(device)
    return real_image.reshape(batch_size, -1), label

def get_test_batch(batch_size, device):
    real_image, label = next(iter(real_image_test))
    real_image = real_image.to(device)
    label = torch.nn.functional.one_hot(label, num_classes=8).type(torch.float).to(device)
    return real_image.reshape(batch_size, -1), label
...
class Generator(nn.Module):
    ...
    def __init__(self, latent_dim=100, context_dim=8, output_dim=28 * 28,leaky_relu=False):
...
class Discriminator(nn.Module):
    ...
    def __init__(self, input_dim=28 * 28, context_dim=8):
